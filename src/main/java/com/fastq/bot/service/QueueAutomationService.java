 ...
 * <li><b>Monitor Queue</b> → poll until
 * {@code current_queue > 0 AND <= threshold}</li>
 ...
 * <li><b>HTTP 403</b> → KILL SWITCH: mark account as ERROR, stop
 * immediately</li>
...
public class QueueAutomationService {
    ...
    private void processAccount(AccountEntity account) throws Exception {
        ...
        SubmitQueueResponse submitResponse = executeWithRetry(() -> apiClient.submitQueue(
                account.getTargetBoardToken(),
                account.getUserToken(),
                eligibleLineId,
                account.getCustomerQty(),
                account.getProxyHost(),
                account.getProxyPort()), account, "SubmitQueue");

        // SUCCESS!
        account.setQueueId(String.valueOf(submitResponse.getQueueId()));
    ...
    private String monitorQueueUntilReady(AccountEntity account) throws Exception {
        ...
        while (true) {
            BoardListResponse boardList = executeWithRetry(() -> apiClient.checkBoardList(
                    account.getUserToken(),
                    account.getLatitude(),
                    account.getLongitude(),
                    account.getProxyHost(),
                    account.getProxyPort()), account, "BoardList");

            if (boardList.getBoardList() != null) {
                for (BoardDTO boad : boardList.getBoardList()) {
                    // Only process the target shop
                    if (!boad.getBoardToken().equals(account.getTargetBoardToken())) {
                        continue;
                    }
                    ...
                    if (isOpen && currentQueue > 0 && currentQueue <= threshold) {
                        // Return the first available queue line ID
                        if (boad.getQueueLineList() != null && !boad.getQueueLineList().isEmpty()) {
                            return String.valueOf(boad.getQueueLineList().get(0).getQueueLineId());
                        } else {
                            log.warn("[{}] Target shop is open but has no queue lines configured.", email);
                        }
                    }
                }
            }
    ...
    private void performLogin(AccountEntity account) throws Exception {
        LoginResponse loginResponse = executeWithRetry(() -> apiClient.login(
                account.getEmail(),
                account.getPassword(),
                account.getProxyHost(),
                account.getProxyPort()), account, "Login");
     ...
     * <ul>
     * <li>HTTP 5xx → Retry up to {@code maxRetryAttempts} with exponential
     * backoff</li>
